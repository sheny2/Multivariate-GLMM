...
# REML fit (two-level multivariate LMM)
# Random INTERCEPT at site; residual exchangeable across outcomes
peal_profile_exch_RI <- function(Y, X, id.site, Z = NULL,
                              init = list(beta = NULL,
                                          log_sigma2 = NULL,
                                          log_tau2 = NULL,
  ...
  safe_chol <- function(S) {
    ...
    stop("Cholesky failed; covariance may be ill-conditioned.")
  }
  chol_solve  <- function(ch, B) { y <- forwardsolve(t(ch), B, upper.tri = FALSE)
  backsolve(ch, y, upper.tri = TRUE) }
  chol_logdet <- function(ch) 2 * sum(log(diag(ch)))

  # ---- Negative RESTRICTED log-likelihood (profiled over beta) ----
  npll_REML <- function(phi, return_beta = FALSE, want_info = FALSE) {
    par <- unpack_par(phi)
    sigma2 <- par$sigma2; tau2 <- par$tau2; rho <- par$rho
    ...
    XtVinvy <- numeric(p_tot)
    logdetV <- 0.0
    # const becomes (n - p) * log(2π)
    # we'll add it after we know p_tot

    # First pass: accumulate GLS normal equations and log|V|
    WX_list <- vector("list", H)  # optional cache if you want, not needed here
    Wy_list <- vector("list", H)
    for (h in seq_len(H)) {
      bh <- site_blocks[[h]]
      yv <- bh$y
      Xb <- bh$X
    ...
    # Quadratic term with profiled beta: sum_h r_h' V_h^{-1} r_h
    quad <- 0.0
    for (h in seq_len(H)) {
      bh <- site_blocks[[h]]
      yv <- bh$y
    ...
    # REML penalty: + log|X' V^{-1} X|
    ch_X <- safe_chol(XtVinvX)$ch
    logdet_XtVinvX <- chol_logdet(ch_X)

    # Restricted log-likelihood
    ll_REML <- -0.5 * ((n - p_tot) * log(2 * pi) + logdetV + logdet_XtVinvX + quad)

    if (return_beta && want_info) {
      Vbeta <- tryCatch(solve(XtVinvX), error = function(e) MASS::ginv(XtVinvX))
      return(list(npll = -ll_REML, beta_hat = beta_hat, XtVinvX = XtVinvX, Vbeta = Vbeta))
    }
    if (return_beta) return(list(npll = -ll_REML, beta_hat = beta_hat))
    -ll_REML
  }

  # ---- Optimize variance parameters (REML) ----
  phi0 <- c(log_sigma2_0, log_tau2_0, eta_rho_0)
  opt  <- optim(phi0, npll_REML, method = "BFGS", control = control)

  # Estimates at REML optimum
  par_hat  <- unpack_par(opt$par)
  beta_out <- npll_REML(opt$par, return_beta = TRUE, want_info = TRUE)
  beta_hat <- beta_out$beta_hat
  Vbeta    <- beta_out$Vbeta
  se_beta  <- sqrt(pmax(diag(Vbeta), 0))
  ...
  structure(list(
    ...
    sigma_u2   = par_hat$tau2,     # site random-intercept variance
    rho        = par_hat$rho,
    logLik_REML= as.numeric(-opt$value),
    criterion  = "REML",
    convergence= opt$convergence,
    counts     = opt$counts,
    message    = opt$message,
    R          = R,
    p_per_outcome = p_r
  ), class = "peal_reml_exch_RI")
}

print.peal_profile_exch_RI <- function(x, ...) {
  cat("peal_reml_exch_RI fit (REML; direct Sigma)\n")
  cat(sprintf("  Outcomes (R): %d\n", x$R))
  cat(sprintf("  REML logLik: %.3f  |  convergence: %s\n",
              x$logLik_REML, as.character(x$convergence)))
  cat(sprintf("  sigma^2: %.6f,  sigma_u^2: %.6f,  rho: %.6f\n",
              x$sigma2, x$sigma_u2, x$rho))
  cat("  beta (stacked by outcomes):\n")
  print(drop(x$beta))
  cat("  se(beta):\n")
  print(drop(x$se_beta))
  invisible(x)
}
